...
REVIEW_DIR = OUT_DIR / "two_source_rife_interpolated_v6_review"
ALL_FRAME_REVIEW_DIR = OUT_DIR / "v6_all_frame_ghost_review"
OUTPUT_VIDEO = OUT_DIR / "scathach_two_source_rife_interpolated_2160p60_4s_v6_highpass_deghost.mp4"
...
def build_highpass_risk_mask(size: tuple[int, int] = (OUT_W, OUT_H)) -> np.ndarray:
...
def build_highpass_cleanup_mask_from_risk(source_a: np.ndarray, source_b: np.ndarray, risk_mask: np.ndarray) -> np.ndarray:
...
def build_highpass_cleanup_mask(source_a: np.ndarray, source_b: np.ndarray) -> np.ndarray:
    return build_highpass_cleanup_mask_from_risk(
        source_a,
        source_b,
        build_highpass_risk_mask((source_a.shape[1], source_a.shape[0])),
    )
...
def apply_highpass_deghost(
    ...
    frame_soft = np.asarray(frame_image.filter(ImageFilter.GaussianBlur(radius)), dtype=np.float32)
    ref_soft = np.asarray(ref_image.filter(ImageFilter.GaussianBlur(radius)), dtype=np.float32)
    frame_high = frame - frame_soft
    ref_high = reference - ref_soft
    cleaned = frame_soft + frame_high * (1.0 - gate) + ref_high * gate
...
def prepared_context(
    ...
    highpass_cleanup_mask_4k = build_highpass_cleanup_mask(source_a_4k, source_b_4k)
    return (
        source_a_1080,
        source_b_1080,
        source_a_4k,
...
def render_frame_4k(
    ...
    frame = apply_highpass_deghost(frame, source_a_4k, source_b_4k, highpass_cleanup_mask_4k, alpha, strength=0.94)
...
def write_all_frame_review(overwrite_sources: bool = False, review_dir: Path = ALL_FRAME_REVIEW_DIR) -> list[Path]:
    ...
    for index in range(TARGET_FRAMES):
        ...
        if index % 20 == 0:
            print(f"reviewed v6 crop frame {index + 1}/{TARGET_FRAMES}", flush=True)
...
def encode_video(overwrite_sources: bool = False, output: Path = OUTPUT_VIDEO) -> Path:
    ...
    try:
        for index, motion_path in enumerate(motion_frames[:TARGET_FRAMES]):
            ...
            if index % 20 == 0:
                print(f"streamed v6 4k frame {index + 1}/{TARGET_FRAMES}", flush=True)
    finally:
        proc.stdin.close()
...
def write_project(video_path: Path, preview_path: Path = REVIEW_DIR / "rife_interpolated_v6_preview.jpg") -> None:
    PROJECT_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(video_path, PROJECT_VIDEO)
    if preview_path.exists():
        shutil.copy2(preview_path, PROJECT_DIR / "preview.jpg")
    project = {
        "title": "Scathach Two Source RIFE Interpolated Loop V6 Highpass Deghost",
        "description": "Looping 2160p60 4s wallpaper using v4 motion plus high-frequency source detail replacement for faint edge trails.",
        "type": "video",
